...
import { overallGpa, qualityPoints } from './gpa';

export interface GpaHistoryPoint {
  t: string;
  score: number;
  // Each class's share of the unrounded average, so differences add up even
  // when the number of graded classes changes between syncs.
...
export function buildGpaHistory(courses: Course[], snapshots: CourseSnapshot[], runs: SyncRun[]): GpaHistoryPoint[] {
  ...
  for (const run of completedRuns) {
    ...
    const grades = tracked.map((course) => ({
      id: course.id,
      score: latest.get(course.id) ?? null,
      level: course.level ?? 'Regular',
    }));
    const score = overallGpa(grades);
    if (score != null) {
      const quality = grades.map((grade) => ({ id: grade.id, points: qualityPoints(grade.score, grade.level) }));
      const count = quality.filter((grade) => grade.points != null).length;
      points.push({
        t: time,
        score,
        shares: Object.fromEntries(quality.map((grade) => [grade.id, grade.points == null ? 0 : grade.points / count])),
      });
    }
  }
  return points;
...
export function buildGpaTimeline(courses: Course[], snapshots: CourseSnapshot[], runs: SyncRun[]): { t: string; score: number }[] {
  return collapseUnchangedPoints(buildGpaHistory(courses, snapshots, runs));
}
